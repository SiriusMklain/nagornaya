	...
	BX.Sale.OrderAjaxComponent = {
		...
		createPaySystemItem: function(item)
		{
			...
			if (logotype && logotype.src_2x)
			{
				logoNode.setAttribute('style',
					'background-image: url(' + logotype.src_1x + ');' +
					'background-image: -webkit-image-set(url(' + logotype.src_orig + ') 1x, url(' + logotype.src_orig + ') 2x)'
				);
			}
			else
			{
				logotype = logotype && logotype.src_orig || this.defaultPaySystemLogo;
				logoNode.setAttribute('style', 'background-image: url(' + logotype + ');');
			}
			label = BX.create('DIV', {
		...
		editPaySystemInfo: function(paySystemNode)
		{
			...
			if (currentPaySystem)
			{
				...
				{
					logoNode.setAttribute('style',
						'background-image: url(' + logotype.src + ');' +
						'background-image: -webkit-image-set(url(' + logotype.src + ') 1x, url(' + logotype.src + ') 2x)'
					);
				}
				else
				{
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			title = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-block'},
				children: [
					BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}, html: currentDelivery.DESCRIPTION.replace('отметьте место', 'укажите на карте точный адрес')}),
					currentDelivery.CALCULATE_DESCRIPTION
						? BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}, html: currentDelivery.CALCULATE_DESCRIPTION.replace('отметьте место', 'укажите на карте точный адрес')})
						: null
				]
			});
		...
		createDeliveryItem: function(item)
		{
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company col-lg-3 col-sm-3 col-xs-6'},
				children: [label, title],
