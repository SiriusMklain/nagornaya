...
<div class="row">
	...
	<h4 class="text-primary mb-3 text-center"><?= Loc::getMessage('MAIN_AUTH_FORM_HEADER');?></h4>

	<form name="<?= $arResult['FORM_ID'];?>" method="post" target="_top" action="<?= POST_FORM_ACTION_URI;?>">

        <div class="mb-4">
            <input class="form-control form-control--light" placeholder="Логин" type="text" name="<?= $arResult['FIELDS']['login'];?>" maxlength="255" value="<?= \htmlspecialcharsbx($arResult['LAST_LOGIN']);?>" />
        </div>

        <div class="mb-4">
            <input id="password"  class="form-control form-control--light" placeholder="Пароль" type="password" name="<?= $arResult['FIELDS']['password'];?>" maxlength="255" autocomplete="off" />
        ...
        <div class="d-grid">
            <?if ($arResult['AUTH_REGISTER_URL']):?>
                <a class="btn btn-primary btn-sm mb-2" href="<?= $arResult['AUTH_REGISTER_URL'];?>" rel="nofollow">
                    <?= Loc::getMessage('MAIN_AUTH_FORM_URL_REGISTER_URL');?>
                </a>
            <?endif;?>
            <input type="submit" class="btn btn-outline-primary btn-sm" name="<?= $arResult['FIELDS']['action'];?>" value="<?= Loc::getMessage('MAIN_AUTH_FORM_FIELD_SUBMIT');?>" />
        </div>
