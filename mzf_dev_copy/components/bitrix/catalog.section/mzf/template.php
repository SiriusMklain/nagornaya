...
$generalParams = array(
	...
	'USE_PRODUCT_QUANTITY' => $arParams["USE_ELEMENT_COUNTER"],
	'PRODUCT_QUANTITY_VARIABLE' => "N",
		...
		foreach ($arResult['ITEM_ROWS'] as $rowData)
		{
			...
			<div class="row <?=$rowData['CLASS']?>" data-entity="items-row">
				...
				switch ($rowData['VARIANT'])
				{
					...
					case 2:
						?>
						<div class=" <?=($arParams["DISPLAY_MODE"] == "line")?'col-xs-12':'row'?> product-item-small-card ">
                            
							<div class="<?=($arParams["DISPLAY_MODE"] == "line")?'row':''?>">
								<?
								foreach ($rowItems as $item)
								{
									?>
									<div class="col-sm-<?=($arParams["DISPLAY_MODE"] == "line")?'12':'4'?> product-item-big-card ">
										<div class="row">
											<div class=" <?=($arParams["DISPLAY_MODE"] == "line")?'':'col-md-12'?>">
												<?
												$APPLICATION->IncludeComponent(
													'bitrix:catalog.item',
													'sp',
													array(
														'RESULT' => array(
															'ITEM' => $item,
															'AREA_ID' => $areaIds[$item['ID']],
															'TYPE' => ($arParams["DISPLAY_MODE"] == "line")?'LINE':'CARD',
															'BIG_LABEL' => 'N',
															'BIG_DISCOUNT_PERCENT' => 'N',
															'BIG_BUTTONS' => 'Y',
															'SCALABLE' => 'N',
                                        'NO_CACHE' => ($arParams["type_display"] == "line")?'Y':'N',
														),
														'PARAMS' => $generalParams
															+ array('SKU_PROPS' => $arResult['SKU_PROPS'][$item['IBLOCK_ID']])
													),
