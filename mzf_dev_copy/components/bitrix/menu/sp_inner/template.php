...
<?if (!empty($arResult)):?>
<ul class="inner_vertical_menu" id="vertical-multilevel-menu">
...
foreach($arResult as $arItem):?>
	...
	<li><a href="<?= str_replace('/product/', '/', $arItem["LINK"])?>" class="near_burger <?if ($arItem["SELECTED"]):?>root-item-selected<?else:?>root-item<?endif?>"><?=$arItem["TEXT"]?>
</a>
				<ul class="root-item">
		<?else:?>
                                    <li><a href="<?= str_replace('/product/', '/', $arItem["LINK"])?>" class="parent<?if ($arItem["SELECTED"]):?> item-selected<?endif?>"><?=$arItem["TEXT"]?>
		...
		<?if ($arItem["PERMISSION"] > "D"):?>
			...
			<?if ($arItem["DEPTH_LEVEL"] == 1):?>
					...
					<li><a href="javascript:void(0);" class="<?if ($arItem["SELECTED"]):?>root-item-selected<?else:?>root-item<?endif?>"><?=$arItem["TEXT"]?></a></li>
                                    <?}else{?>
                                    <li><a href="<?= str_replace('/product/', '/', $arItem["LINK"])?>" class="<?if ($arItem["SELECTED"]):?>root-item-selected<?else:?>root-item<?endif?>"><?=$arItem["TEXT"]?></a></li>

                                    <?}?>
                                            <?else:?>
					<li><a href="<?= str_replace('/product/', '/', $arItem["LINK"])?>" <?if ($arItem["SELECTED"]):?> class="item-selected"<?endif?>><?=$arItem["TEXT"]?></a></li>
			<?endif?>
