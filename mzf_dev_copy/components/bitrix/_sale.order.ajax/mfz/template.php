...
if (strlen($request->get('ORDER_ID')) > 0) {
    ...
    ?><? /*
      ...
      document.cookie = "ORDER_PRICE=<?=$arResult["ORDER_DATA"]["ORDER_PRICE"]?>;";
      document.cookie = "DELIVERY_PRICE=<?=$arResult["ORDER_DATA"]["PRICE_DELIVERY"]?>;";





      //    'transactionAffiliation': 'Московская зеркальная фабрика',
      //    'transactionTotal': '',
      //    'transactionShipping': '<?=$arResult["ORDER_DATA"]["PRICE_DELIVERY"]?>',
      //    'transactionProducts':
      //    <?foreach($arResult["BASKET_ITEMS"] as $key => $item){
      $cookie_items .= ">>".$item["PRODUCT_ID"]."|".$item["NAME"]."|".$item["BASE_PRICE"]."|".$item["QUANTITY"];
      }?>
      //; path = /personal/order/";
      document.cookie = "ITEMS=<?=$cookie_items?>";
    ...
    ?>
        <input type="hidden" name="form_application" value="Заказ">
    ...
    <? else: ?>
                    ...
                    <div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active">
                        <div class="bx-soa-section-title-container">
                            <h2 class="bx-soa-section-title col-sm-9">
                                <span class="bx-soa-section-title-count"></span><?= $arParams['MESS_PAYMENT_BLOCK_NAME'] ?>  (Доставка изделий из интернет-магазина возможна только при 100% предоплате изделий и доставки.)
                            </h2>
                            <div class="col-xs-12 col-sm-3 text-right"><a href="" class="bx-soa-editstep"><?= $arParams['MESS_EDIT'] ?></a></div>
    ...
    <? endif ?>
                ...
                <!--	ORDER SAVE BLOCK	-->
                <div id="bx-soa-total" class="col-sm-12 bx-soa-sidebar">
                ...
                <div id="bx-soa-orderSave">
                    Скидка предоставляется только на товар в наличии
                    <div class="checkbox">
    <?
...
<?
/* if ($USER->IsAdmin()){?>
  <pre><?print_r($arResult["ORDER_DATA"])?></pre>
  <?} */?>