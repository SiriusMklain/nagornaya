	...
	BX.Sale.OrderAjaxComponent = {
		...
		getBlockFooter: function(node)
		{
				...
				buttons.push(
					BX.create('A', {
						props: {
							...
							className: 'pull-left btn btn-primary btn-sm mt-4 d-block'
						},
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {href: 'javascript:void(0)', className: 'pull-right btn btn-primary btn-sm mt-4 d-block'},
		...
		createPaySystemItem: function(item)
		{
			...
			logoNode = BX.create('div', {
				props: {className: 'block-pay'},
				html: "<div class=\"form-check\">\n" +
					"                                                        <div class=\"col-sm-6\">\n" +
					"                                                            <h6>"+item.NAME+"</h6>\n" +
					"                                                        </div>\n" +
					"                                                        <div class=\"col-sm-6\">\n" +
					"                                                            <img src=\""+logotype.src_1x+"\" class=\"img-fluid\">\n" +
					"                                                        </div>\n" +
					"                                                    </div>"
				});
			...
			*/
			label = BX.create('DIV', {
				props: {className: 'card card--primary h-100'},
				children: [
					BX.create('INPUT', {
						props: {
...
*/
			...
			if (currentPaySystem)
			{
				...
				title = BX.create('DIV', {
					...
					children: [BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc', style: 'font-size: 16px;'}, html: currentPaySystem.DESCRIPTION})]
				});
...
/*
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-pp-item-container row', style: 'margin-top: 0px;'}}),
				deliveryItemNode, k;
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-pp-desc-container', style: 'position: absolute;margin-top: -17px;width: 95%;'}}),
			...
			title = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-block'},
				children: [
					BX.create('DIV', {props: {className: 'bx-soa-pp-company-desc'}, html: currentDelivery.DESCRIPTION.replace('отметьте место','укажите адрес')}),
				...
				price = BX.create('LI', {
					children: [
						...
						BX.create('DIV', {
							...
							children: this.getDeliveryPriceNodes(currentDelivery) + ' руб.'
						})
					]
				});
			}
...
/*
		...
		createDeliveryItem: function(item)
		{
			...
			logoNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-image2'},
				html:"<div class=\"form-check\">\n" +
					"                                                        <div class=\"col-sm-6\">\n" +
					"                                                            <h6>"+item.NAME+" ("+item.PRICE_FORMATED+")</h6>\n" +
					"                                                        </div>\n" +
					"                                                        <div class=\"col-sm-6\">\n" +
					"                                                            <img src=\"/local/templates/mzf_dev_copy/images/new/delivery.png\" class=\"img-fluid\">\n" +
					"                                                        </div>\n" +
					"                                                    </div>"
			});
			/*
			...
			*/
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company delivery-item col-lg-6 col-sm-6 col-xs-6'},
				children: [label],
				events: {click: BX.proxy(this.selectDelivery, this)}
			});
			checked && BX.addClass(itemNode, 'bx-selected checkedDeliver');
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'col-lg-6 bx-soa-customer'}}),
		...
		{
			var propsMapContainer = BX.create('DIV', {props: {className: 'col-sm-6'}, style: {marginBottom: '10px'}}),
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = BX.create('DIV', {props: {className: 'col-sm-6'}});
			label = BX.create('LABEL', {
			...
			input = BX.create('input', {
				props: {
					id: 'orderDescription',
					cols: '4',
					className: 'form-control bx-soa-customer-textarea bx-ios-fix deliverOrder',
		...
		editTotalBlock: function()
		{
			...
			// 					props: {
			// 						href: 'javascript:void(0)',
			// 						className: 'btn btn-primary btn-sm d-block btn-order-save'
			// 					},
		...
		editMobileTotalBlock: function()
		{
			if (this.result.SHOW_AUTH)
				...
				BX.addClass(this.mobileTotalBlockNode, 'hidden-total');
		...
		{
			var totalValue, className = 'bx-soa-cart-total-line list-attribute__item';
			...
			return BX.create('DIV', {
				...
				children: [
					BX.create('SPAN', {props: {className: 'list-attribute__name'}, text: name}),
					BX.create('SPAN', {props: {className: 'list-attribute__line'}}),
					BX.create('SPAN', {
						props: {
							className: 'list-attribute__value' + (!!params.total && this.options.totalPriceChanged ? ' bx-soa-changeCostSign' : '')
						},
						children: totalValue
					})
				]
		...
		totalBlockScrollCheck: function()
		{
			...
			if (scrollTop > ghostTop && !BX.hasClass(this.totalInfoBlockNode, ''))
			{
				width = this.totalInfoBlockNode.offsetWidth;
				BX.addClass(this.totalInfoBlockNode, '');
			...
			else if (scrollTop < ghostTop && BX.hasClass(this.totalInfoBlockNode, ''))
			{
				BX.removeClass(this.totalInfoBlockNode, '');
		...
		totalBlockResizeCheck: function()
		{
			...
			if (BX.hasClass(this.totalInfoBlockNode, ''))
