...
$(".postsms").click(function () {


    $('.postcod').removeClass('visually-hidden');


    $(this).addClass("disabled"); // После нажатия, чтобы получить проверочный код, отключаем кнопку и запускаем обратный отсчет
    var time = 300; // Время обратного отсчета, произвольное
    var $this = $(this); // Резервное копирование, таймер асинхронный, это не то
    var timer = setInterval(function () {
        time--; // Запуск обратного отсчета
        if (time == 0) { // Когда обратный отсчет составляет 0 секунд, выключите таймер, измените текст на кнопке и сделайте его интерактивным
            clearInterval(timer);
            $this.val("получить код подтверждения");
            $this.removeClass("disabled");
            return;
        }
        $this.val(time + "Восстановлено за секунды"); // Отображаем оставшиеся секунды
    }, 1000); // Таймер срабатывает раз в секунду, и каждый раз, когда он уменьшается на единицу, он ведет обратный отсчет


    var formData = new FormData();

    let phone = $('input[name="ORDER_PROP_3"]').val();

    formData.append('phone', phone);
    $.ajax({
        url: "/testtel/postsms.php",
        type: "POST",
        dataType: "json",
        cache: false,
        contentType: false,
...
$(".postcode").click(function () {
    ...
    let codesms = $('.codesms').val();
    let phone = $('input[name="ORDER_PROP_3"]').val();

    formData.append('codesms', codesms);
    formData.append('phone', phone);

    $.ajax({
        url: "/testtel/postcode.php",
        type: "POST",
        dataType: "json",
        cache: false,
        contentType: false,
