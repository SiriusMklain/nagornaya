...
$item['DETAIL_PAGE_URL'] = $ar_res['SECTION_PAGE_URL'].$item["~CODE"].'/';
	...
	<? endif; ?>
		...
		<span class="product-item-image-original" id="<?=$itemIds['PICT']?>"
			style="background-image:linear-gradient(180deg, rgba(0, 0, 0, 0.5) 0%, rgba(0, 0, 0, 0) 36.26%),  url('<?=$item['PREVIEW_PICTURE']['SRC']?>'); <?=($showSlider ? 'display: none;' : '')?>">
		</span>
		<?
		if ($item['SECOND_PICT'])
		{
			...
			<span class="product-item-image-alternative" id="<?=$itemIds['SECOND_PICT']?>"
				style="background-image: linear-gradient(180deg, rgba(0, 0, 0, 0.5) 0%, rgba(0, 0, 0, 0) 36.26%), url('<?=$bgImage?>'); <?=($showSlider ? 'display: none;' : '')?>">
			</span>
			<?
		}
